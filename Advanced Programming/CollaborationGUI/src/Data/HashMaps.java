...
public class HashMaps implements Serializable {
	...
	//can't remove because we can remove teams, but not projects
	private HashMap<String,Team> teams = new HashMap<String, Team>();
	private HashMap<String,Team> tempTeams = new HashMap<String, Team>();
	private PreferenceCalc prefCalc = new PreferenceCalc();
	//change to Student
	private HashMap<String,Student> unassignedStudents = new HashMap<String, Student>();
...
private static HashMaps _instance;
	...
	private void calculateAverageGrade() {
		average.calcAndSetAverageGrade(students);
	...
	public HashMap<String,StudentInfoWithPrefs> getStudentInfoWithPrefs() {
		HashMap<String,StudentInfoWithPrefs> studs = new HashMap<String,StudentInfoWithPrefs>();
		for(Map.Entry entry : students.entrySet()) {
			studs.put((String)entry.getKey(), (StudentInfoWithPrefs) entry.getValue());
		}
		return studs;
	}
	...
	public void removeTeamMember(String teamId, String studentId) {
		...
		unassignedStudents.put(studentId, (StudentInfoWithPrefs) students.get(studentId));
	}
	...
	public void addPreferences(Preferences prefs) throws IdAlreadyUsedException {
		...
		prefCalc.addFour(prefs.getProjectPreferences().get(0));
		prefCalc.addThree(prefs.getProjectPreferences().get(1));
		prefCalc.addTwo(prefs.getProjectPreferences().get(2));
		prefCalc.addOne(prefs.getProjectPreferences().get(3));
	}
	...
	public void addStudInfo(StudentInfo sInfo) throws IdAlreadyUsedException {
		...
		students.put(sInfo.getID(), sInfo);
	}	
	...
	public StudentInfoWithPrefs getStudentInfoWPrefs(String student) throws InvalidStudentException {
		if (students.containsKey(student)) {
			return (StudentInfoWithPrefs) students.get(student);
		}
		else {
	...
	public void setStudentInfoWPrefs(HashMap<String,Student> studentInfoWPrefs) {
	...
	public void mergeStudentInfoAndPreferences() throws IncorrectPersonalityTypeException, IncorrectGradeException, IncorrectRangeException {
		for(Map.Entry entry : students.entrySet()) {
			if (prefs.containsKey(entry.getKey())) {
				...
				students.put((String) entry.getKey(),withPrefs);
			}
		}
	}
	public void addStudentInfoWithPreferences(StudentInfoWithPrefs studentWPrefs) throws IncorrectPersonalityTypeException, IncorrectGradeException, IncorrectRangeException {
		students.put(studentWPrefs.getID(),studentWPrefs);
	}
	...
	public void addTeam(Team team) throws InvalidMemberException {
	   ...
	   public void addAllStudentsToUnassignedStudentsHashMap() {
		   unassignedStudents.putAll(students);
		   for(int i=0;i<students.size();i++) {
	   ...
	   public ArrayList<Student> getArrayStudents() {
		...
		public ArrayList<StudentInfoWithPrefs> getArrayStudentInfoWPrefs() {
				   ArrayList<StudentInfoWithPrefs> values = new ArrayList<StudentInfoWithPrefs>();
					students.entrySet().forEach(entry->{
					    values.add((StudentInfoWithPrefs) ( entry).getValue());  
					 }); //hm
				   return values;  
		} 
	...
	private HashMap<String, ProjectObjectiveFunction> addleaderStudents(DefaultObjectiveWeighting objWeights, HashMap<String, Student> unassignedStuds) {
	...
	private void removeUnassignedStudents(HashMap<String, Student> unassignedStuds, HashMap<String, ProjectObjectiveFunction> leaderMap) {
	...
	private HashMap<String,ProjectObjectiveFunction> addStudents(DefaultObjectiveWeighting objWeights,HashMap<String, Student> unassignedStuds) {
	...
	public String autoCalculateObjectiveFunction(DefaultObjectiveWeighting weighting) {
		...
		HashMap<String,Student> unassignedStuds = new HashMap<String,Student>();

		unassignedStuds.putAll(students);
		average.calcAndSetAverageGrade(students);
		// leaders
		...
		double newObjValue = Calculations.round2DecPl(calculateObjectiveFunction(weighting, tempTeams));
	...
	private String stringAlgorithmsOutput(double newObjectiveFunction, DefaultObjectiveWeighting functionWeights) {
		...
		double originalObjective = Calculations.round2DecPl(calculateObjectiveFunction(functionWeights, teams));
	...
	public ArrayList<String> returnKeysToRemove(Integer projectsToRemove) {
		return prefCalc.returnKeysToRemove(projectsToRemove);
	}
