...
public class MatchTeamWithMembers extends Project implements Serializable{
	...
	public MatchTeamWithMembers(Project proj,DefaultObjectiveWeighting objWeights){
		super(proj);
	...
	public void compareUnassignedLeaders(HashMap<String, Student> unassignedStuds,double overallAverageGrade) {
		//start from fresh options
		remainingOptions.clear();
		// O(n) students array complexity
		unassignedStuds.entrySet().forEach(entry->{
			StudentInfoWithPrefs student = (StudentInfoWithPrefs) ( entry).getValue();
			if(student.isLeader()) {
		...
		QuickSortStudent qs = new QuickSortStudent();
	...
	public void compareUnassignedConstrained(HashMap<String, Student> unassignedStuds,double overallAverageGrade) {
		//start from fresh options
		remainingOptions.clear();
		...
		Iterator it = unassignedStuds.entrySet().iterator();
		// O(n) students array complexity
		while(it.hasNext()) {
