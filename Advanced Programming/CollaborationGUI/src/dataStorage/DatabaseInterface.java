...
import Objects.SQLObjects.ProjectSQL;
import Objects.SQLObjects.StudentSQL;
...
public void insertAllObjectsIntoSQL(HashMaps hm) {
	   	insertAllStudents(hm.getArrayStudentInfoWPrefs());
...
private static void createCompanies() {
   ...
   private static void createStudents() {
	   ...
	   try {
		   while (rs.next()) {
				...
				h1.addStudentInfoWithPreferences(new StudentSQL(rs));
   ...
   private static void insertAllStudents(ArrayList<StudentInfoWithPrefs> studs) {
	   ...
	   for (int i = 0; i<studs.size();i++) {
		   StudentSQL stud = new StudentSQL(studs.get(i));
		   sql += "(" + stud.getSQL() + ")" ;
		   if(i==studs.size()-1) {
			   sql += ";";
		   } else {
   ...
   private static void insertCompanies(ArrayList<Company> companies) {
	   ...
	   for (int i = 0; i<companies.size();i++) {
		   CompanySQL c = new CompanySQL(companies.get(i));
	   ...
	   executeUpdate(sql);
	   } catch (ABNValuesIncorrectException | ABNLengthIncorrectException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
   }
   private static void insertOwners(ArrayList<Owner> owners) {
	   ...
	   for (int i = 0; i<owners.size();i++) {	
		   OwnerSQL o = new OwnerSQL(owners.get(i));
		   if (owners.size()-1==i) {
			   sql +=  o.getSQL() + ";";				   
		   } else {
			   sql +=  o.getSQL() + ",";					   
   ...
   private static void insertProjects(ArrayList<Project> projects) {
	   ...
	   for (int i = 0; i<projects.size();i++) {
		   ProjectSQL proj = new ProjectSQL(projects.get(i));
...
private static void insertShortlist(ArrayList<Project> arrayShortList) {
	   ...
	   for (int i = 0; i<arrayShortList.size();i++) {	
		   ProjectSQL proj = new ProjectSQL(arrayShortList.get(i));
		   if (arrayShortList.size()-1==i) {
			   sql +=  proj.getShortlistSQL() + ";";				   
		   } else {
			   sql +=  proj.getShortlistSQL() + ",";					   
		   }
	   }
