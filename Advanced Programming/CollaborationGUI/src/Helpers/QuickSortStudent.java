...
public class QuickSortStudent
{
    ...
    public ArrayList<StudentInfoWithPrefs> sort(ArrayList<StudentInfoWithPrefs> array) {
    	sort(array, 0, array.size()-1);
    	return array;
    } // end of sort()

    
    protected void sort(ArrayList<StudentInfoWithPrefs> array, int leftIndex, int rightIndex) {
    	...
    	// find median
    	int medianIndex;
    	if (array.get(leftIndex).getOf().getObjectiveValue() <= array.get(midIndex).getOf().getObjectiveValue()) {
    		if (array.get(midIndex).getOf().getObjectiveValue() <= array.get(rightIndex).getOf().getObjectiveValue()) {
    			medianIndex = midIndex;
    		}
    		else if (array.get(leftIndex).getOf().getObjectiveValue() <= array.get(rightIndex).getOf().getObjectiveValue()) {
    	...
    	else if (array.get(midIndex).getOf().getObjectiveValue() <= array.get(rightIndex).getOf().getObjectiveValue()) {
    		medianIndex = rightIndex;
    	}
    	else {
    		medianIndex = midIndex;
